...
if (isset($_POST['db_host'])
    ...
    $search_arr = ["TABLE addressbook"
        , "TABLE month_lookup"
        , "TABLE group_list"
        , "TABLE address_in_groups"
        , "TABLE users"
    ];
    $replac_arr = ["TABLE " . $_POST['db_tbl_prefix'] . "addressbook"
...
if (!$db) {
include_once __DIR__ . "/format.inc.php";
...
<div id="container">
    <div id="top"></div>
    <div id="header"><a href="."><img width="340" height="75" src="title_x2.png" title="%ADDRESS_BOOK%"
                                      alt="%ADDRESS_BOOK%"/></a></div>
    <div id="nav"></div>
    <div id="content">
        <form accept-charset="utf-8" method="post">
            <label>Host: </label><input name='db_host'
                                        value='<?= (isset($_POST['db_host']) ? $_POST['db_host'] : 'localhost'); ?>'><br>
            <label>Name: </label><input name='db_name'
                                        value='<?= (isset($_POST['db_name']) ? $_POST['db_name'] : 'test'); ?>'><br>
            <label>User: </label><input name='db_user'
                                        value='<?= (isset($_POST['db_user']) ? $_POST['db_user'] : 'root'); ?>'><br>
            <label>Pass: </label><input name='db_pass'
                                        value='<?= (isset($_POST['db_pass']) ? $_POST['db_pass'] : ''); ?>'><br>
            <label>Prefix: </label><input name='db_tbl_prefix'
                                          value='<?= (isset($_POST['db_tbl_prefix']) ? $_POST['db_tbl_prefix'] : ''); ?>'><br>
            <label>History: </label><input type=checkbox
                                           name='db_hist' <?= (isset($_POST['db_hist']) && $_POST['db_hist'] != '' ? 'checked=checked' : ''); ?>
            '><br>
            <input type='submit'>
        </form>
        <?php
