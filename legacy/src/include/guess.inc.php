...
function guessAddressFields($address)
{
    ...
    if (!isset($company_exts)) $company_exts = [];
    if (!isset($title_exts)) $title_exts = [];
    ...
    for ($i = 0, $iMax = count($addr_list); $i < $iMax; $i++) {
        $addr_line = trim($addr_list[$i]);
        $addr_line = str_replace(["(at)", "{at}", "'at'", "'dot'"], ["@", "@", "@", "."], $addr_line);
        ...
        if ($guessed_month != false
            ...
            elseif (preg_match("/(\+)?([0-9\(\)])+([0-9\(\) -\/'])*/", $addr_line, $matches) > 0) {
                ...
                // Mobil(e), Natel, Cell
                $phone_exts = ["TEL M", "M", "MOBIL", "MOBILE", "N", "NATEL", "C", "CELL"];
                if (preg_match("/^(" . implode("|", $phone_exts) . ")(\.)?(:)?(\s)+/", $addr_line_upper, $matches) > 0) {
                    $phone_type = "CELL";
                }

                // Gesch�ft, Business
                $phone_exts = ["TEL G", "G", "GESCH�FT", "B", "BUSINESS"];
                if (preg_match("/^(" . implode("|", $phone_exts) . ")(\.)?(:)?(\s)+/", $addr_line_upper, $matches) > 0) {
                    $phone_type = "WORK";
                }

                // Fax, Facsmile
                $phone_exts = ["F", "FAX", "TELEFAX"];
                if (preg_match("/^(" . implode("|", $phone_exts) . ")(\.)?(:)?(\s)+/", $addr_line_upper, $matches) > 0) {
                    $phone_type = "FAX";
                }
    ...
    //
    $phone_types = ['WORK', 'FAX', 'HOME', 'CELL'];
    $phone_targets = ['work', 'fax', 'home', 'mobile', 'phone2'];

    for ($i = 0, $iMax = count($phone_types); $i < $iMax; $i++) {
    ...
    if (isset($phones['']) && count($phones['']) > 0) {
        ...
        $phone_type = '';
        for ($j = 1, $jMax = count($phones[$phone_type]); $j < $jMax; $j++) {
