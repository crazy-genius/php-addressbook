...
class AuthLoginFactory
{
    ...
    static function getBestLogin($required_roles = [])
    {
...
class AuthLoginIP extends AuthLoginImpl
{
    ...
    function __construct($whitelist, $blacklist = [])
    {
    ...
    function __construct($db_conn, $table)
    {
        ...
        if ($cnt == 0 && $this->getUserName() != "") {
            ...
            $sql = "select user_id, domain_id, username, md5_pass from " . $table
                . " where username in ('" . mysqli_real_escape_string($db, $username) . "','"
                . mysqli_real_escape_string($db, $username_lower) . "')"
